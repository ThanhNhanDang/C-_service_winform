...
namespace Service_RFID_BIG_READER.Reader_DLL
{
    internal class SwitchDLL : InterfaceRfidAPI
    {
        ...
        public void Inventory()
        {
            ...
            if (readerType == ReaderType.CF_RU6403)
            {
                if (ConfigMapping.ARR_ANTENNA_IN.Count != 0)
                    for (int inAtn = ConfigMapping.ARR_ANTENNA_IN.Count - 1; inAtn >= 0; inAtn--)
                        cF_RU6403_DLL_DEFINE.Inventory((byte)(0x80 | ConfigMapping.ARR_ANTENNA_IN[inAtn]));
                if (ConfigMapping.ARR_ANTENNA_OUT.Count != 0)
                    for (int outAtn = ConfigMapping.ARR_ANTENNA_OUT.Count - 1; outAtn >= 0; outAtn--)
