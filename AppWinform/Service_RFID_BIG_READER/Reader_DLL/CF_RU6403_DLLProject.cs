...
namespace Service_RFID_BIG_READER.Reader_DLL
{
    public class CF_RU6403_DLL_DEFINE
    {
        ...
        public CF_RU6403_DLL_DEFINE()
        {
            ...
            this.ANT = Convert.ToByte(ConfigurationManager.AppSettings["antenna"]);
            if (TIDFlag == 1)
            {
                tidAddr = 2;
                tidLen = 4;
        ...
        private void WriteAntenna(byte ANT)
        {
        ...
        private async Task<bool> CheckTime(DateTime dateTime, string key, string keyCondition, string valueCondition)
        {
        ...
        private bool CheckTimer(DateTime dateTime)
        {
            DateTime now = DateTime.Now;
            double diffInSeconds = (now - dateTime).TotalSeconds;
            //Kiểm tra xem thẻ đó đã được free chưa (thời gian cập nhật lần cuối)
        ...
        public async void GetUid(IntPtr p, Int32 nEvt)
        {
            ...
            if (dto != null)
            {
                ...
                bool isIn = false;
                if (ConfigMapping.ARR_ANTENNA_OUT.Contains((byte)Antnum))
                {
                ...
                if (ce.UID == dto.tidNg)
                {
                    ...
                    if (dto.isInNg == isIn && await CheckTime(dto.lastUpdate, "lastUpdate", "tidNg", ce.UID))
                    {
                        isNg = true;
                        tagInfo = new TagInfo(DateTime.Now, dto.epcNg, dto.tidNg, dto.passNg, "crc", ce.RSSI.ToString(), "ant", topic, isNg, isIn);
                        if (!_tagInfoNgDic.ContainsKey(tagInfo.tid))
                        {
                            System.Diagnostics.Debug.WriteLine("tidNg");
                ...
                {
                    // Nếu thẻ xe chưa vào
                    if (dto.isInXe == isIn && await CheckTime(dto.lastUpdateXe, "lastUpdateXe", "tidXe", ce.UID))
                    {
                        isNg = false;
                        tagInfo = new TagInfo(DateTime.Now, dto.epcXe, dto.tidXe, dto.passXe, "crc", ce.RSSI.ToString(), "ant", topic, isNg, isIn);
                        if (!_tagInfoXeDic.ContainsKey(tagInfo.tid))
                        {
                            System.Diagnostics.Debug.WriteLine("tidXe");
        ...
        private void EncPass()
        {
            TagInfo tagInfo = new TagInfo();

            if (_tagInfoNgDic.Any())
            {
                foreach (KeyValuePair<string, TagInfo> keyValuePair in _tagInfoNgDic)
                {
                    tagInfo = keyValuePair.Value;
                    if (EncPass(tagInfo.password, tagInfo.epc))
                    {
                        Service1.PublishMessage(JsonConvert.SerializeObject(tagInfo), tagInfo.topic);
                        _tagInfoNgDic.Remove(keyValuePair.Key);
                        if (!_tagInfoNgDic.Any()) break;
                    }
                    else
                        Service1.PublishMessage(JsonConvert.SerializeObject(tagInfo), (tagInfo.topic == TopicPub.OUT_MESSAGE) ? TopicPub.OUT_FAIL_ENC_PASS : TopicPub.IN_FAIL_ENC_PASS);
                }
            }
            if (_tagInfoXeDic.Any())
            {
                foreach (KeyValuePair<string, TagInfo> keyValuePair in _tagInfoXeDic)
                {
                    tagInfo = keyValuePair.Value;
                    if (EncPass(tagInfo.password, tagInfo.epc))
                    {
                        Service1.PublishMessage(JsonConvert.SerializeObject(tagInfo), tagInfo.topic);
                        _tagInfoXeDic.Remove(keyValuePair.Key);
                        if (!_tagInfoXeDic.Any()) break;
                    }
                    else
                        Service1.PublishMessage(JsonConvert.SerializeObject(tagInfo), (tagInfo.topic == TopicPub.OUT_MESSAGE) ? TopicPub.OUT_FAIL_ENC_PASS : TopicPub.IN_FAIL_ENC_PASS);
                }
